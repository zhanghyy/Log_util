    ...
    def get_logger_and_add_handlers(self, log_level_int: int = None, *, is_add_stream_handler=True,
       ...
       """
        if log_level_int is None:
            log_level_int = log_config_default.LOG_LEVEL_FILTER
        if do_not_use_color_handler is None:
            do_not_use_color_handler = not log_config_default.DEFAULUT_USE_COLOR_HANDLER
        if log_filename is None and log_config_default.DEFAULT_ADD_MULTIPROCESSING_SAFE_ROATING_FILE_HANDLER:
            log_filename = f'{self._logger_name}.log'
        if log_file_size is None:
            log_file_size = log_config_default.LOG_FILE_SIZE
        if log_path is None:
            log_path = log_config_default.LOG_PATH or '/pythonlogs'
        if formatter_template is None:
            formatter_template = log_config_default.FORMATTER_KIND
        ...
        self._log_file_handler_type = log_file_handler_type or log_config_default.LOG_FILE_HANDLER_TYPE

        if isinstance(formatter_template, int):
            self._formatter = log_config_default.FORMATTER_DICT[formatter_template]
        ...
        if not (self._judge_logger_has_handler_type(ConcurrentRotatingFileHandler) or
            ...
            if self._log_file_handler_type in (1, None):
                if os_name == 'nt':
                    ...
                    if log_config_default.LOG_FILE_CACHE:
                        file_handler = ConcurrentRotatingFileHandlerWithBufferInitiativeWindwos(filename=log_file,
                                                                                            maxBytes=self._log_file_size * 1024 * 1024,
                                                                                            backupCount=log_config_default.LOG_FILE_BACKUP_COUNT,
                                                                                            encoding="utf-8")
                    else:
                    ...
                    if log_config_default.LOG_FILE_CACHE:
                        ...
                        file_handler = ConcurrentRotatingFileHandlerWithBufferInitiativeLinux(filename=log_file,
                                                                                          maxBytes=self._log_file_size * 1024 * 1024,
                                                                                          backupCount=log_config_default.LOG_FILE_BACKUP_COUNT,
                                                                                          encoding="utf-8")
                    else:
                ...
                file_handler = ConcurrentDayRotatingFileHandler(self._log_filename, self._log_path,
                                                                back_count=log_config_default.LOG_FILE_BACKUP_COUNT)
